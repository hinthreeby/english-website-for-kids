...
/* ── Same colorful star data as LoginPage ─────────────────── */
const STARS = [
...
const RegisterPage = () => {
  ...
  const [form, setForm] = useState({
    role: "parent", username: "", email: "", password: "", confirmPassword: "",
  });
  ...
  const handleVerifySubmit = async (e) => {
    ...
    if (code.length < CODE_LENGTH) { setError(t("register.verify.enterAllDigits")); return; }
    ...
    return (
      <div className="lp-page">
        ...
        <div className="lp-card-wrap">
          <form className="lp-card" onSubmit={handleVerifySubmit} style={{ gap: "14px" }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: "2.8rem", lineHeight: 1, marginBottom: "6px" }}>📬</div>
              <h2 style={{ margin: "0 0 6px", color: "#ffd700", fontFamily: "'Fredoka One','Baloo 2',sans-serif", fontSize: "1.7rem" }}>
                {t("register.verify.title")}
              </h2>
              <p style={{ margin: "0 0 4px", color: "#c4b5fd", fontSize: "13.5px" }}>
                {t("register.verify.subtitle")} <strong style={{ color: "#e9d5ff" }}>{form.email}</strong>
              </p>
              <p style={{ color: "rgba(196,181,253,0.6)", fontSize: "12px", margin: 0 }}>
            ...
            <button type="button" className="lp-btn-register" onClick={handleResend} disabled={resendCooldown > 0 || resendLoading}>
              {resendLoading ? t("register.verify.sending")
                : resendCooldown > 0 ? t("register.verify.resendCooldown", { count: resendCooldown })
  ...
  return (
    <div className="lp-page">
      ...
      <div className="lp-card-wrap lp-card-wrap-wide">
        <form className="lp-card" onSubmit={handleFormSubmit}>
          ...
          <div className="lp-card-header">
            ...
            <p className="lp-subtitle">Create your space explorer account 🚀</p>
          </div>

          {/* Role selector */}
          <div className="rp-role-grid" role="radiogroup" aria-label="Choose role">
            {[
              { id: "parent",  emoji: "👨‍👩‍👧", label: t("register.roleParent")  },
              { id: "teacher", emoji: "👩‍🏫",   label: t("register.roleTeacher") },
            ].map((opt) => (
              <button key={opt.id} type="button"
                className={`rp-role-pill${form.role === opt.id ? " rp-role-pill--active" : ""}`}
                onClick={() => setForm((p) => ({ ...p, role: opt.id }))}>
          ...
          <div className="rp-fields-grid">
            ...
            <div className="rp-field">
              <label className="rp-label">{t("register.username")}</label>
              <div className="lp-input-group">
                <span className="lp-input-icon"><IconUser /></span>
                <input className="lp-input" placeholder={t("register.usernamePlaceholder")}
                  value={form.username} minLength={2} autoComplete="username"
                  onChange={setField("username")} required />
              </div>
            </div>

            {/* Email */}
            <div className="rp-field">
              <label className="rp-label">{t("register.email")}</label>
              <div className="lp-input-group">
                <span className="lp-input-icon"><IconMail /></span>
                <input className="lp-input" placeholder="you@example.com"
            ...
            <div className="rp-field">
              ...
              <div className="lp-input-group">
                ...
                <input className="lp-input" placeholder={t("register.passwordPlaceholder")}
                  type={showPassword ? "text" : "password"}
                  value={form.password} minLength={4} autoComplete="new-password"
                  style={{ paddingRight: "44px" }}
                  onChange={setField("password")} required />
            ...
            {/* Confirm password */}
            <div className="rp-field">
              <label className="rp-label">{t("register.confirmPassword")}</label>
              <div className="lp-input-group">
                <span className="lp-input-icon"><IconLock /></span>
                <input className="lp-input" placeholder={t("register.confirmPasswordPlaceholder")}
                  type={showConfirmPassword ? "text" : "password"}
                  value={form.confirmPassword} minLength={4} autoComplete="new-password"
                  style={{ paddingRight: "44px" }}
                  onChange={setField("confirmPassword")} required />
                <button type="button" className="lp-pw-toggle"
